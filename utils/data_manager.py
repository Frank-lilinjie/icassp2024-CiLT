...
class DataManager(object):
    def __init__(self, dataset_name,datatype, shuffle, seed, init_cls, increment):
        self.dataset_name = dataset_name
        self._setup_data(dataset_name,datatype, shuffle, seed)
    ...
    def _setup_data(self, dataset_name,datatype,shuffle, seed):
        idata = _get_idata(dataset_name)
        idata.download_data()

        # Data
        self._train_data, self._train_targets = ImbalanceDatasets(idata.train_data, idata.train_targets, datatype)
...
def ImbalanceDatasets(train_data, train_targets, datatype):
    imb_factor = 0.1
    new_data = []
    new_targets = []
    cls_num = len(np.unique(train_targets))
    img_max = len(train_data) / cls_num
    ...
    if datatype == 'conv':
        ...
        if datatype == 'lt':
            ...
            for cls_idx in reversed(range(cls_num)):
                num = img_max * (imb_factor**(cls_idx / (cls_num - 1.0)))
                img_num_per_cls.append(int(num))

    for i, the_img_num in zip(range(cls_num), img_num_per_cls):
        num_per_cls_dict[i] = the_img_num
    
    for i in range(cls_num):
        idx = np.where(targets_np == i)[0]
        np.random.shuffle(idx)
        selec_idx = idx[:num_per_cls_dict[i]]
        new_data.append(train_data[selec_idx, ...])
        new_targets.extend([i, ] * num_per_cls_dict[i])
