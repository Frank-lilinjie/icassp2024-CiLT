...
class IncrementalNet(BaseNet):
    def __init__(self, args, pretrained, gradcam=False):
        ...
        self.old_centroids = torch.tensor([]).to(self.device)
    ...
    def initialize_new_centroids(self, num_new_classes):
        self.new_centroids = nn.Parameter(torch.randn(num_new_classes, self.out_dim).to(self.device))
        self.old_centroids = self.old_centroids.detach()  # Ensure old centroids are detached
    
    def end_task(self):
        """To be called at the end of a task to move new_centroids to old_centroids."""
        if self.old_centroids.nelement() == 0:  # If old_centroids is empty
            self.old_centroids = self.new_centroids.detach().clone()
        else:
            self.old_centroids = torch.cat([self.old_centroids, self.new_centroids.detach().clone()], dim=0)
        
        # Remove new_centroids or reset it for the next task
        self.new_centroids = None
