...
class iCaRL_2stage(BaseLearner):
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        centroids = torch.randn(self._total_classes-self._known_classes, self._network.feature_dim).to(self._device)  # 假设你的模型输出的特征向量的维度是 feature_dim
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                intra_class_cos_sim = F.cosine_similarity(features, centroids[targets])# 计算特征向量与其对应类别质心的余弦相似度
                intra_class_loss = -intra_class_cos_sim.mean()
                inter_class_cos_sim = F.cosine_similarity(centroids.unsqueeze(0), centroids.unsqueeze(1)).mean()
                inter_class_loss = inter_class_cos_sim
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss = loss_clf + loss_kd+ inter_class_loss + intra_class_loss
