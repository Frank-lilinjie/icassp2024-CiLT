...
class BaseLearner(object):
    ...
    def eval_task(self, save_conf=False):
        ...
        # figure.show_confusion(y_pred, y_true, self._total_classes)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                # confidences = compute_confidences(outputs).cpu().detach().numpy()
                # for t, c, f in zip(targets.cpu().numpy(), confidences, features):
        ...
        # sample_silhouette_values = silhouette_samples(all_features, all_labels)
        # unique_labels = np.unique(all_labels)
        # class_avg_silhouette = {}
        # for label in unique_labels:
        #     class_avg_silhouette[label] = sample_silhouette_values[np.array(all_labels) == label].mean()
        # for class_id, silhouette_avg in class_avg_silhouette.items():
        #     logging.info(f"Class {class_id} Average Silhouette Coefficient: {silhouette_avg:.3f}")
        
        # figure.visualize_tsne_features(class_features, self._cur_task)
        # figure.plot_avg_confidences_per_class(class_confidences, self._cur_task)
...
def compute_confidences(logits):
    """
